...
{
    init_graphics();
    resize_graphics(winwidth, winheight);
...
GLint object_hit(circle *aliens,line *missile){
...
GLint object_hitship(circle *aliens,int pos){
    int y=50;
    int y1=10;
    if(y>=aliens->v1.y-5 &&
       y<=aliens->v1.y+5 &&
...
void draw(void)
{
    ...
    draw_graphics(num_aliens, CrlSegments, num_bombs, bombs, num_missile, LineSegments, currentpos, currenty, ang);
}
